...
export class UserSettingService {
  ...
  getByState(state:any){
    return this.http.get<any[]>(`${ConfigService.toApiUrl("user-settings-by-state")}/${state}`,ConfigService.addAction('LISTER'));
  }
  ...
  setState(id:any,state:any){
    return this.http.get<any[]>(`${ConfigService.toApiUrl("user-settings-set-state")}/${id}/state/${state}`,ConfigService.addAction('CHANGER_STATUT'));
  }
}
